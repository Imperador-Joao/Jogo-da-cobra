...
def novo_jogo():
    ...
    global RODANDO,GAME_OVER,legenda
    global cobra,comida,direcao,pontuacao
    global bota2,maior_pontuacao_legenda,maior_pontuacao
    ...
    if not RODANDO:
        ...
        maior_pontuacao_legenda.config(text=f'Maior pontuação: {maior_pontuacao}')
...
def game_over():
    ...
    global RODANDO,GAME_OVER,pontuacao,maior_pontuacao
    ...
    if pontuacao > maior_pontuacao:
        ...
        quadro.create_text(quadro.winfo_width()/2,quadro.winfo_height()/2 - 150,
                           font = ('Comic Sans',30),text = 'VOCÊ BATEU O RECORDE!',fill='red',tag='recorde')

        with open('Pontuações.txt','w') as arquivo_pontuacoes:
            arquivo_pontuacoes.write(str(maior_pontuacao))

    return

with open('Pontuações.txt','r') as arquivo_pontuacoes:

    maior_pontuacao = int(arquivo_pontuacoes.readline())
...
legenda = Label(janela,text= f'Pontuação: {pontuacao}',font = ('consolas',30),
                bg = FUNDO,fg = 'white')
legenda.pack()

maior_pontuacao_legenda = Label(janela,text= f'Maior pontuação: {maior_pontuacao}',font = ('consolas',13),
                bg = FUNDO,fg = 'white')

maior_pontuacao_legenda.pack()
